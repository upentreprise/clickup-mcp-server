...
async function main() {
  ...
  try {
    if (config.enableSSE) {
      console.log('--> [DEBUG] SSE mode enabled, starting server...');
      startSSEServer();
    } else {
      console.log('--> [DEBUG] STDIO mode enabled, starting server...');
      await startStdioServer();
    }
  } catch (err) {
    console.error('!!! A FATAL ERROR OCCURRED IN MAIN CATCH BLOCK !!!');
    console.error(err);
    process.exit(1);
  }
}
